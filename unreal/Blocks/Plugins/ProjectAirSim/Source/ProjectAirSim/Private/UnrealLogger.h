...
//   UE::Core::TCheckedFormatString<TCHAR, ArgTypes...> checked(format);


//   FString LogMessage = FString::Printf(checked, args...);
...
#include "String/FormatStringSan.h"  // for UE::Core::TCheckedFormatString

class UnrealLogger {
 public:
  template <typename... TArgs>
  static void Log(microsoft::projectairsim::LogLevel level,
                  UE::Core::TCheckedFormatString<FString::FmtCharType, std::remove_cvref_t<TArgs>...> format,
                  TArgs&&... args);
...
inline void UnrealLogger::Log(microsoft::projectairsim::LogLevel level,
                              ...
                              TArgs&&... args)
{
  const FString LogMessage = FString::Printf(format, Forward<TArgs>(args)...);

  switch (level) {
    case microsoft::projectairsim::LogLevel::kFatal:   UE_LOG(SimPlugin, Fatal,   TEXT("%s"), *LogMessage); break;
    case microsoft::projectairsim::LogLevel::kError:   UE_LOG(SimPlugin, Error,   TEXT("%s"), *LogMessage); break;
    case microsoft::projectairsim::LogLevel::kWarning: UE_LOG(SimPlugin, Warning, TEXT("%s"), *LogMessage); break;
    case microsoft::projectairsim::LogLevel::kTrace:   UE_LOG(SimPlugin, Log,     TEXT("%s"), *LogMessage); break;
    case microsoft::projectairsim::LogLevel::kVerbose: UE_LOG(SimPlugin, Verbose, TEXT("%s"), *LogMessage); break;
    default:                                           UE_LOG(SimPlugin, VeryVerbose, TEXT("%s"), *LogMessage); break;
  }

  auto time_stamp = GetTimeStamp();
  std::clog << "[" << time_stamp << "] " << TCHAR_TO_UTF8(*LogMessage) << std::endl;
}
...
inline void UnrealLogger::LogSim(const std::string& module,
                                 ...
                                 const std::string& message)
{
  // Works with UE’s checked formatting (%hs for narrow char*):
  Log(level, TEXT("[%hs] %hs"), module.c_str(), message.c_str());
  // Alternatively (avoids %hs by preconverting):
  // Log(level, TEXT("[%s] %s"), *FString(module.c_str()), *FString(message.c_str()));
}
